...
const Day5Flow = ({ campaignId, campaignTitle, isAdmin, completed, onBack, onComplete, onNavigateNext }: Day5FlowProps) => {
        ...
        <div key={step} className={cn("animate-in duration-300 px-5 max-w-lg mx-auto", direction === "right" ? "slide-in-from-right-8" : "slide-in-from-left-8")}>
          ...
          {step === 1 && (
            <MediaSlider
              ...
              instruction="Súbelos a tu estado Y mándalos directo a tus clientas trabajadoras"
              totalSlots={2}
              assets={videoAssets}
              ...
              inspirationMessages={[
                "Amiga, ya es viernes 🎉 mira este look con tennis importados — perfecto para salir cómoda y guapa después del trabajo 👟✨",
...
const Step1Mission = () => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8 py-8">
    ...
    <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground leading-tight">
      Hoy vas con las que trabajan
    </h1>
    <p className="text-base text-muted-foreground leading-relaxed max-w-sm">
      El viernes es el día que más piensan en el fin de semana — mándales el look perfecto antes de que salgan. Tennis importados con outfit casual: cómodas, guapas y listas para lo que venga.
    </p>
    <p className="text-sm italic max-w-xs" style={{ color: "hsl(330, 85%, 55%)" }}>
      "El viernes es tuyo — aprovecha que ya están pensando en salir."
    </p>
  </div>
);
