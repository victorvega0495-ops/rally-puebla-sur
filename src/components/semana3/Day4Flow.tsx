...
const Day4Flow = ({ campaignId, campaignTitle, isAdmin, completed, onBack, onComplete, onNavigateNext }: Day4FlowProps) => {
  ...
  const stepLabels = ["La misión", "Tu video", "Tus imágenes", "Resumen"];
        ...
        <div key={step} className={cn("animate-in duration-300 px-5 max-w-lg mx-auto", direction === "right" ? "slide-in-from-right-8" : "slide-in-from-left-8")}>
          ...
          {step === 1 && (
            <SingleVideoSlot
              ...
              onUpload={(file) => uploadAsset(file, "video_0", 0, setVideoAssets, setVideoUploading)}
              onRemove={() => removeAsset("video_0", 0, setVideoAssets, videoAssets)}
...
const Step1Mission = () => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8 py-8">
    ...
    <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground leading-tight">
      Hoy vas con las familias
    </h1>
    <p className="text-base text-muted-foreground leading-relaxed max-w-sm">
      Los looks en familia generan más reacciones que cualquier otro contenido — todo mundo quiere verse bien con sus hijos. Hoy subes el contenido a tu estado y lo mandas directo a mamás y papás.
    </p>
    <p className="text-sm italic max-w-xs" style={{ color: "hsl(330, 85%, 55%)" }}>
      "Una foto en familia vende sola — tú solo tienes que mandarla."
    </p>
  </div>
);

/* ========== SINGLE VIDEO SLOT (Paso 2) ========== */
interface SingleVideoSlotProps {
  assets: Record<number, { url: string; fileName: string } | null>;
...
const SingleVideoSlot = ({ assets, uploading, isAdmin, campaignId, inputRefs, onUpload, onRemove, onShare, onDownload }: SingleVideoSlotProps) => {
  ...
  return (
    <div className="flex flex-col items-center min-h-[60vh] py-4 -mx-5">
      <div className="text-center space-y-2 px-5 mb-4">
        <h1 className="font-display text-2xl font-bold text-foreground">Tu video de hoy</h1>
        <p className="text-sm text-muted-foreground">Súbelo a tu estado Y mándalo directo a mamás y papás</p>
      </div>

      <div className="relative w-full flex-1 flex items-center justify-center">
        {isAdmin && (
          <button onClick={() => inputRefs.current[0]?.click()} className="absolute top-3 right-5 z-10 w-8 h-8 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-black/70">
            <Upload className="w-4 h-4" />
          </button>
        )}
        {isAdmin && asset && (
          <button onClick={onRemove} className="absolute top-3 right-16 z-10 w-8 h-8 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-black/70">
            <X className="w-4 h-4" />
          </button>
        )}
        ...
        {isUploading ? (
          ...
          <div
            ...
            onClick={() => isAdmin && inputRefs.current[0]?.click()}
          >
        ...
        <input ref={(el) => { inputRefs.current[0] = el; }} type="file" accept=".mp4,.mov,.webm,.avi" className="hidden"
      ...
      {isAdmin && (
        <div className="px-5 w-full max-w-sm mt-3">
          <ProductMetaInputs campaignId={campaignId} dayNumber={DAY} assetType="video_0" />
        </div>
      )}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8 py-8">
      ...
      <div className="w-full max-w-xs space-y-3">
        {phase === "idle" && (
          <button style={gradientStyle} onClick={handleTap}>✅ ¡Día completado!</button>
        )}
        {phase === "green" && (
